...
export default class DraggableSVG extends Subject {
	...
	_apply(actionName) {
		...
		if (actionName === 'resize') {
			...
			if (!['text'].includes(element.tagName.toLowerCase())) {
				element.setAttribute(
					'transform',
...
function applyResize(element, data) {
	...
	case 'text': {
		const x = element.x.baseVal[0].value,
			y = element.y.baseVal[0].value;
		...
		// ];
		attrs.push(
			['data-ratio-x', resX + (scaleX < 0 ? boxW : 0)],
			['data-ratio-y', resY + (scaleY < 0 ? boxH : 0)]
		);
		break;
	}
	case 'circle': {
