...
const Palette = ({ width, height, gradientData, onPointMove }) => {
  ...
  return (
    <div className='palette' style={{ width, height }}>
      ...
      >
        <GradientDef gradientId={gradientId} gradientData={gradientData} />
        <rect x='0' y='0' width='100%' height='100%' fill={gradientUrl} />
        {[
          { x: StartX, y: StartY, text: 'S' },
          { x: EndX, y: EndY, text: 'E' }
        ].map((point) => {
          return (
            <DraggableCircle
              ...
              {...{ onDrag }}
              transform={`matrix(1,0,0,1,${point.x * width},${
                point.y * height
              })`}
            >
              <circle fill='black' r={8} />
              <text x={-4} y={4} fill='white'>
                {point.text}
              </text>
            </DraggableCircle>
          );
