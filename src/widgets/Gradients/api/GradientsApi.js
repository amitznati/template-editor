...
export default class GradientsApi extends BaseApi {
	...
	removeGradientFromLayout = () => {
		const editTemplateMainViewApi = getInstance().EditTemplateMainViewApi;
		const {selectedLayout} = editTemplateMainViewApi.getSelectedLayoutSelector();
		selectedLayout.properties.fill = {...selectedLayout.properties.fill, fill: '', gradientId: null};
	...
	onAddGradient = (item) => {
		...
		const editTemplateMainViewApi = getInstance().EditTemplateMainViewApi;
		const {selectedLayout} = editTemplateMainViewApi.getSelectedLayoutSelector();
		selectedLayout.properties.fill = {...selectedLayout.properties.fill, fill: `url(#${gradient.id})`, gradientId: gradient.id};
		editTemplateMainViewApi.onUpdateLayout(selectedLayout);
	};
