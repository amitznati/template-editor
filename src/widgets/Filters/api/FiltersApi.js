...
import { toCamelCaseString } from '../../../sdk/utils';
...
export default class FiltersApi extends BaseApi {
  ...
  removeFilterFromLayout = (filterId) => {
    ...
    selectedLayout.properties.filters = selectedLayout.properties.filters.filter(
      (f) => f !== filterId
    );
    editTemplateMainViewApi.onUpdateLayout(selectedLayout);
  };
  ...
  onIgnoreAttribute = ({ name, primitiveIndex, filterId, childIndex }) => {
    ...
    const newFilters = filters.map((f) => {
      if (f.id === filterId) {
        if (!isNaN(childIndex)) {
          f.primitives[primitiveIndex].children[childIndex].params[
            name
          ].isIgnore = !f.primitives[primitiveIndex].children[childIndex]
            .params[name].isIgnore;
        } else {
          f.primitives[primitiveIndex].params[name].isIgnore = !f.primitives[
            primitiveIndex
          ].params[name].isIgnore;
        }
      }
  ...
  onSortEnd = ({ parentFilterId, oldIndex, newIndex, filterIndex }) => {
    ...
    if (!isNaN(filterIndex)) {
      newFilters[filterIndex].children = arrayMove(
        filters[filterIndex].children,
        oldIndex,
        newIndex
      );
    } else {
  ...
  removeFilterFromTemplate = (filter) => {
    ...
    template.layouts.forEach((l) => {
      l.properties.filters = l.properties.filters.filter(
        (id) => id !== filter.id
      );
    });
  ...
  onAddFilter = (parentFilterId, filterItem) => {
    ...
    const primitives = this.getPrimitivesByFilterId(parentFilterId);
    const sameGroupPrimitives = primitives.filter(
      (f) => f.groupName === filterItem.groupName
    );
  ...
  onAddChildFilter = (filterItem, filterParent, parentFilterId) => {
    ...
    const childFilter = filterData.children.find(
      (f) => f.groupName === filterItem.groupName
    );
    const clonedChild = JSON.parse(JSON.stringify(childFilter));
    const sameGroupFilters = filterParent.children.filter(
      (f) => f.groupName === filterItem.groupName
    );
    const filters = this.getPrimitivesByFilterId(parentFilterId);
    const newFilters = filters.map((f) => {
  ...
  mergeImportedToData = (importedElement, primitive) => {
    ...
    importedElement.attributes &&
      Object.keys(importedElement.attributes).forEach((attrName) => {
        primitive.params[toCamelCaseString(attrName.replace(':', '-'))] = {
          value: importedElement.attributes[attrName],
          isIgnore: false
        };
      });
  };
  ...
  mapPrimitive = (filter) => {
    ...
    const filterObjElements = filter.elements.filter(
      (el) => el.type === 'element'
    );
    filterObjElements.forEach((el) => {
      ...
      if (groupNameObj) {
        ...
        primitive.id =
          (el.attributes && el.attributes.result) || primitive.groupName;
        this.mergeImportedToData(el, primitive);
        if (el.elements && primitive.children) {
          const primitivesChildrenNameList = this.getChildrenFiltersNamesList(
            groupNameObj
          );
          const childFilterObjElements = el.elements.filter(
            (el) => el.type === 'element'
          );
          ...
          if (hasSingleChild) {
            ...
            childFilterObjElements.forEach((childEl) => {
              const childGroupNameObj = primitivesChildrenNameList.find(
                (p) => p.name === childEl.name
              );
            ...
            primitive.children = childFilterObjElements.map((childEl) => {
              ...
              const childPrimitive = this.getChildData(
                childGroupNameObj.groupName,
                primitive.groupName
              );
  ...
  mapFilterObj = (filterObj) => {
    if (
      !filterObj ||
      !filterObj.elements ||
      filterObj.elements.length > 1 ||
      filterObj.elements[0].type !== 'element' ||
      filterObj.elements[0].name !== 'filter' ||
      !filterObj.elements[0].elements ||
      filterObj.elements[0].elements.length === 0
    )
      return false;
    ...
    Object.keys(filter.attributes).forEach((attrName) => {
      filterToReturn.params[attrName] = {
        value: filter.attributes[attrName],
        isIgnore: false
      };
    });
  ...
  getChildData = (childGroupName, parentGroupName) => {
    ...
    const childPrimitive = parentPrimitive.children.find(
      (c) => c.groupName === childGroupName
    );
